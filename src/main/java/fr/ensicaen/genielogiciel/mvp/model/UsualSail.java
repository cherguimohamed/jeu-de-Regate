...
public class UsualSail extends BoatDecorator {
    private final double SLOWDOWN = 0.1;
    public UsualSail(Boat b) {
        super(b);
    }
    ...
    public void move(double windDirection, double windStrength) {
        ...
        if(super.isVirementDeBord(windDirection)) {
            super.setDx(super.getVelocityPolarMatrix()[row][col] * -Math.cos(super.getDirection() * Math.PI / 180) * 0.1 * SLOWDOWN);
